...
def init_personality_table():
    ...
    with sqlite3.connect(DB_FILE) as conn:
        ...
        """)
        conn.commit()
    logger.info("  Personality profiles table ready.")
...
"""
    ...
    try:
        ...
        logger.warning("  LLM personality analysis failed for %s: %s", contact, e)
...
async def analyze_personality(
    ...
    if existing and existing.get("posts_analyzed", 0) > 0:
        logger.info("  Fresh profile exists for %s - skipping.", contact)
        return existing
    ...
    try:
        ...
        logger.warning("  Could not parse posts for %s: %s", contact, e)

    if not posts:
        logger.warning("  No posts found for %s", contact)
        return {}
